...
const MatchingPage: React.FC = () => {
  ...
  const getItemClass = (word: string) => {
    ...
    let base = "w-full p-4 mb-3 rounded-2xl font-bold text-center border-2 transition-all duration-200 transform active:scale-95 ";
    
    if (isMatched) return base + "bg-emerald-50 border-emerald-500 text-emerald-700 opacity-60 cursor-default";
    if (isWrong) return base + "bg-rose-50 border-rose-500 text-rose-700 animate-shake";
    if (isSelected) return base + "bg-orange-50 border-orange-500 text-orange-700 shadow-md -translate-y-1";
    
    return base + "bg-white border-slate-100 text-slate-600 hover:border-orange-200 hover:bg-orange-50/30";
  };
